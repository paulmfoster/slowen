...
class slowen
{
	function __construct($db)
	{
		...
		$this->messages = array(
			'F2101' => 'F2101 Transfers must have a valid payee',
			'F2102' => 'F2102 Transfers must have a valid to account',
			'F2103' => 'F2103 Normal transactions must have a valid payee',
			'F2104' => 'F2104 Normal transactions  must have a valid to account',
			'F2106' => 'F2106 Splits must have a valid to account',
			'S2108' => 'S2108 Transaction(s) stored.',
			'F2115' => "F2115 No amount field found. Amount set to 0.",
			'S2116' => "S2116 Transaction voided",
			'S2117' => "S2117 Transaction UPDATE successful",
			'S2118' => 'S2118 Payee added',
			'S2119' => 'S2119 Payee updated',
			'S2120' => 'S2120 Payee deleted',
			'S2122' => 'S2122 New account has been saved',
			'S2125' => 'S2125 Account deleted',
			'F2129' => "F2129 Payee is linked to transactions. Aborted",
			'F2130' => "F2130 Account is linked to transactions. Aborted",
			'F2131' => "F2131 Transaction is reconciled. Cannot void.",
			'F2132' => "F2132 Attempt to delete non-existent payee. Aborted.",
			'F2133' => "F2133 Cannot edit non-existent payee",
			'F2134' => "F2134 Cannot edit non-existent account",
			'F2135' => "F2135 Cannot delete non-existent account",
			'F2136' => 'F2136 No such origin account',
			'F2137' => 'F2137 No such destination acct/category',
			'F2138' => 'F2138 No such payee',
			'F2139' => "F2139 Split amounts don't add up to transaction amount"
		);

		$this->entity = isset($_SESSION['entity']) ? $_SESSION['entity'] : 1;

	}

	private function message($idx)
	{
		if (isset($this->messages[$idx])) {
			$_SESSION['messages'][] = $this->messages[$idx];
		}
		else {
			$_SESSION['messages'][] = 'Unknown message index ' . $idx;
		}
	}
	...
	{
		if (is_null($last_dt)) {
			$last_dt = date::get(date::now(), 'Y-m-d');
		}
	...
	function add_transaction($post)
	{
		...
		$post['txn_dt'] = date::reformat($date_template, $post['txn_dt'], 'Y-m-d');

		if (!empty($post['recon_dt'])) {
			$post['recon_dt'] = date::reformat($date_template, $post['recon_dt'], 'Y-m-d');
		}
	...
	function update_transaction($post)
	{
		...
		if ($post['txntype'] == 'iaxfer') {
			for ($i = 0; $i < 2; $i++) {
				$txn_dt = date::reformat($date_template, $post['txn_dt'][$i], 'Y-m-d');
	...
	function add_account($post)
	{
		...
		if (!empty($post['open_dt'])) {
			$post['open_dt'] = date::reformat($date_template, $post['open_dt'], 'Y-m-d');
		}
		if (!empty($post['recon_dt'])) {
			$post['recon_dt'] = date::reformat($date_template, $post['recon_dt'], 'Y-m-d');
		}
	...
	function get_account($acct_id)
	{
		...
		$acct['x_open_dt'] = date::reformat('Y-m-d', $acct['open_dt'], 'm/d/y');
		$acct['x_recon_dt'] = date::reformat('Y-m-d', $acct['recon_dt'], 'm/d/y');
	...
	function audit($year, $month)
	{
		...
		$from_dt = date::fromints($year, $month, 1);
		$from_dt = date::adddays($from_dt, -1);
		$to_dt = date::fromints($year, $month, $dims[$month - 1]);

		$start_dt = date::get($from_dt, 'Y-m-d');
		$end_dt = date::get($to_dt, 'Y-m-d');
	...
	function finish_reconciliation($from_acct, $stmt_end_bal, $recon_dt, $ids)
	{
		...
		$recon_dt = date::reformat($date_template, $recon_dt, 'Y-m-d');
	...
	function get_transaction($txnid)
	{
		...
		for ($i = 0; $i < $max_txns; $i++) {
			...
			if (!empty($txns[$i]['recon_dt'])) {
				$txns[$i]['x_recon_dt'] = date::reformat('Y-m-d', $txns[$i]['recon_dt'], 'm/d/y');
			}
			else {
				$txns[$i]['x_recon_dt'] = '';
			}
