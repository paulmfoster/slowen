...
class database
{
	...
	function __construct($cfg)
	{
		...
		case 'sqlite':
		case 'sqlite3':
			require_once('classes/pdosqlite3.lib.php');
			$this->dbh = new pdosqlite3($cfg['dbdata']);
			break;
		case 'pg':
		case 'postgresql':
			require_once('classes/dbpostgresql.lib.php');
			$this->dbh = new dbpostgresql($cfg);
			break;
		case 'mysql':
			require_once('classes/dbmysql.lib.php');
	...
	function delete($table, $where_clause)
	{
		...
		if (empty($where_clause)) {
			$this->fatal('DELETE statement with no where clause');
		}
